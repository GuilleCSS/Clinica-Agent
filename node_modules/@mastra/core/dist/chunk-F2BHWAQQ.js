...
var BackgroundTaskManager = class {
  ...
  async init(pubsub) {
    this.pubsub = pubsub;
    this.workerCallback = async (event, ack, nack) => {
   ...
   */
  async handleDispatch(event, nack) {
    ...
    try {
      ...
      const onProgress = async (chunk) => {
        if (shouldThrottleProgress) {
          const now = Date.now();
          if (lastProgressEmitMs !== void 0 && now - lastProgressEmitMs < progressThrottleMs) {
            return;
          }
      ...
      if (currentTask.maxRetries > 0 && deliveryAttempt <= currentTask.maxRetries) {
        const shouldRetry = this.config.defaultRetries?.retryableErrors ? this.config.defaultRetries.retryableErrors(error) : true;
        if (shouldRetry && nack) {
          await storage.updateTask(taskId, {
            status: "pending",
            error: void 0,
  ...
  async executeWithTimeout(taskId, executor, args, timeoutMs, onProgress) {
...
export { BackgroundTaskManager, backgroundOverrideJsonSchema, backgroundOverrideZodSchema, createBackgroundTask, generateBackgroundTaskSystemPrompt, resolveBackgroundConfig };
//# sourceMappingURL=chunk-F2BHWAQQ.js.map
//# sourceMappingURL=chunk-F2BHWAQQ.js.map