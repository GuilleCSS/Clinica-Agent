...
var MastraLLMV1 = class extends chunkFCQNDFEW_cjs.MastraBase {
  ...
  async __text({
    ...
    const model = this.#model;
    const observabilityContext = chunkCDN65CUN_cjs.resolveObservabilityContext(rest);
    ...
    if (experimental_output) {
      ...
      if (chunkAX6D722J_cjs.isZodType(experimental_output)) {
    ...
    const argsForExecute = {
      ...
      onStepFinish: async (props) => {
        ...
        if (!isNaN(remainingTokens) && remainingTokens > 0 && remainingTokens < 2e3) {
          ...
          await chunkAX6D722J_cjs.delay(10 * 1e3);
        }
      },
      experimental_output: schema ? chunkD6H6ESH4_cjs.output_exports.object({
        schema
      }) : void 0
    };
    try {
      const result = await chunk5IS5R433_cjs.executeWithContext({
        span: llmSpan,
        fn: () => chunkD6H6ESH4_cjs.generateText(argsForExecute)
      });
  ...
  async __textObject({
    ...
    try {
      let output = "object";
      if (chunkRLPRSIR2_cjs.isZodArray(structuredOutput)) {
        output = "array";
        const zodDef = chunkRLPRSIR2_cjs.getZodDef(structuredOutput);
      ...
      try {
        const result = await chunkD6H6ESH4_cjs.generateObject(argsForExecute);
        llmSpan?.end({
          output: {
            object: result.object,
            warnings: result.warnings
  ...
  __stream({
    ...
    if (llmSpan) {
      chunk5IS5R433_cjs.executeWithContextSync({
        span: llmSpan,
    ...
    try {
      return chunk5IS5R433_cjs.executeWithContextSync({ span: llmSpan, fn: () => chunkD6H6ESH4_cjs.streamText(argsForExecute) });
    } catch (e) {
      const mastraError = new chunk4U7ZLI36_cjs.MastraError(
        {
  ...
  __streamObject({
    ...
    try {
      ...
      if (chunkRLPRSIR2_cjs.isZodArray(structuredOutput)) {
        output = "array";
        structuredOutput = chunkRLPRSIR2_cjs.getZodDef(structuredOutput).type;
      }
      const processedSchema = this._applySchemaCompat(structuredOutput);
      llmSpan?.update({
        input: {
      ...
      try {
        return chunkD6H6ESH4_cjs.streamObject(argsForExecute);
      } catch (e) {
        const mastraError = new chunk4U7ZLI36_cjs.MastraError(
          {
...
//# sourceMappingURL=chunk-Y73X6U5P.cjs.map
//# sourceMappingURL=chunk-Y73X6U5P.cjs.map