...
function createBrowserReminderMessage(reminderMarkup, url, title) {
  const reminderMeta = {
    type: REMINDER_TYPE,
    url,
    title
...
function hasTrailingBrowserReminder(messages, url, title) {
  ...
  const reminder = metadata.systemReminder;
...
2. Remove profile/executablePath to connect to the existing browser via CDP`
  ...
  getInputProcessors(configuredProcessors = []) {
    const hasProcessor = configuredProcessors.some(
      (p) => !chunkQPZ35KK2_cjs.isProcessorWorkflow(p) && "id" in p && p.id === "browser-context"
    );
...
Object.defineProperty(exports, "BrowserCliHandler", {
  enumerable: true,
  get: function () { return chunk5ANWVE3P_cjs.BrowserCliHandler; }
});
Object.defineProperty(exports, "browserCliHandler", {
  enumerable: true,
  get: function () { return chunk5ANWVE3P_cjs.browserCliHandler; }
