...
var chunkLUS5VPK2_cjs = require('../../chunk-LUS5VPK2.cjs');
...
var chunkEBDGUBJJ_cjs = require('../../chunk-EBDGUBJJ.cjs');
...
async function resolveScope(agent, mergedOptions) {
  const requestContext = mergedOptions?.requestContext ?? new chunkKF4FCWWL_cjs.RequestContext();
  const memory = await agent.getMemory();
  if (!memory) return null;
  const threadIdFromContext = requestContext.get(chunkKF4FCWWL_cjs.MASTRA_THREAD_ID_KEY);
...
async function runDurableStreamUntilIdle(agent, messages, streamOptions, deps) {
  ...
  const mergedOptions = chunkAX6D722J_cjs.deepMerge(
    defaultOptions,
    restStreamOptions ?? {}
  );
  ...
  const processedCompletionIds = /* @__PURE__ */ new Set();
  const innerCleanups = [];
...
async function prepareForDurableExecution(options) {
  ...
  const requestContext = providedRequestContext ?? new chunkKF4FCWWL_cjs.RequestContext();
  const requestVersions = requestContext.get(chunkKF4FCWWL_cjs.MASTRA_VERSIONS_KEY);
  let mergedVersions = chunkKF4FCWWL_cjs.mergeVersionOverrides(mastra?.getVersionOverrides?.(), requestVersions);
  if (execOptions?.versions) {
    mergedVersions = chunkKF4FCWWL_cjs.mergeVersionOverrides(mergedVersions, execOptions.versions);
  }
  if (mergedVersions) {
    requestContext.set(chunkKF4FCWWL_cjs.MASTRA_VERSIONS_KEY, mergedVersions);
  }
  ...
  const messageList = new chunkB5SLBAGA_cjs.MessageList({
    threadId,
    resourceId
  });
  ...
  if (inputProcessors.length > 0) {
    try {
      ...
      const { ProcessorRunner: ProcessorRunner2 } = await import('../../runner-2AOFRABN.cjs');
  ...
  const saveQueueManager = memory ? new chunkQPZ35KK2_cjs.SaveQueueManager({
    logger,
    memory
  }) : void 0;
...
function createDurableAgentStream(options) {
  ...
  const output = new chunkQPZ35KK2_cjs.MastraModelOutput({
    model,
    stream,
    messageList,
    messageId,
...
function createDurableBackgroundTaskCheckStep() {
  return chunkQPZ35KK2_cjs.createStep({
    id: BG_CHECK_STEP_ID,
...
async function resolveRuntimeDependencies(options) {
  ...
  if (globalEntry) {
    ...
    try {
      ...
      const resolveRequestContext = new chunkKF4FCWWL_cjs.RequestContext();
  ...
  if (memory) {
    saveQueueManager = new chunkQPZ35KK2_cjs.SaveQueueManager({
...
async function resolveModelFromConfig(config, mastra) {
  ...
  if (typeof modelConfigString === "string") {
    return await chunkLUS5VPK2_cjs.resolveModelConfig(modelConfigString, requestContext, mastra);
  }
  return await chunkLUS5VPK2_cjs.resolveModelConfig(
    modelConfigString,
    requestContext,
    mastra
  );
      ...
      for (let modelIndex = 0; modelIndex < modelList.length; modelIndex++) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
          try {
            ...
            if (!chunkQPZ35KK2_cjs.isSupportedLanguageModel(model)) {
            ...
            if (registryEntry?.inputProcessors?.length) {
              ...
              const runner = new chunkQPZ35KK2_cjs.ProcessorRunner({
            ...
            const modelResult = chunkQPZ35KK2_cjs.execute({
              runId,
              model: currentModel,
              inputMessages,
              tools: currentTools,
            ...
            const outputStream = new chunkQPZ35KK2_cjs.MastraModelOutput({
              model: {
            ...
            if (registryEntry?.errorProcessors?.length) {
              try {
                ...
                const currentMessageList = new chunkB5SLBAGA_cjs.MessageList();
  ...
  return chunkQPZ35KK2_cjs.createStep({
    ...
    execute: async (params) => {
      ...
      if (bgManager && !bgConfig?.disabled && typeof cleanedArgs === "object" && cleanedArgs !== null) {
        const bgResolved = chunkEBDGUBJJ_cjs.resolveBackgroundConfig({
          llmBgOverrides,
          toolName,
        ...
        if (bgResolved.runInBackground) {
          try {
            const bgTask = chunkEBDGUBJJ_cjs.createBackgroundTask(bgManager, {
              toolName,
              toolCallId,
              args: cleanedArgs,
    ...
    execute: async ({ inputData }) => {
      ...
      if (toolResults.length > 0) {
        const toolResultParts = toolResults.map((toolResult) => {
          ...
          } else if (toolResult.result !== void 0) {
            resultContent = typeof toolResult.result === "string" ? toolResult.result : JSON.stringify(toolResult.result);
          } else {
            resultContent = "";
          }
          return {
      ...
      const output = {
        ...
        output: {
          text: void 0,
          // Text is in the LLM output, would be extracted from assistant message
...
function createDurableAgenticWorkflow(options) {
  ...
  const singleIterationWorkflow = chunkQPZ35KK2_cjs.createWorkflow({
  ...
  return chunkQPZ35KK2_cjs.createWorkflow({
    ...
    async (params) => {
      ...
      if (registryEntry?.outputProcessors?.length) {
        try {
          ...
          const outputMessageList = new chunkB5SLBAGA_cjs.MessageList();
          outputMessageList.deserialize(state.messageListState);
          await runner.runOutputProcessors(outputMessageList, {}, requestContext ?? new chunkKF4FCWWL_cjs.RequestContext(), 0);
        } catch (error) {
      ...
      if (registryEntry?.saveQueueManager && registryEntry.memory && durableState?.threadId && durableState?.resourceId && !durableState.observationalMemory) {
        try {
          const memoryMessageList = new chunkB5SLBAGA_cjs.MessageList();
    ...
    async (params) => {
      ...
      const logger = mastra?.getLogger?.();
      const inputMessageList = new chunkB5SLBAGA_cjs.MessageList();
      ...
      const resolveContext = requestContext ?? new chunkKF4FCWWL_cjs.RequestContext();
      for (const [scorerKey, scorerEntry] of Object.entries(scorers)) {
        ...
        try {
          ...
          chunkQPZ35KK2_cjs.runScorer({
            ...
            threadId: initData.state?.threadId,
            resourceId: initData.state?.resourceId,
            ...chunkCDN65CUN_cjs.createObservabilityContext(tracingContext)
          });
        } catch (error) {
...
var DurableAgent = class extends chunkQPZ35KK2_cjs.Agent {
  /** The wrapped agent */
  #wrappedAgent;
  ...
  constructor(config) {
    ...
    this.#innerPubsub = pubsub ?? new chunkHM6XXOV2_cjs.EventEmitterPubSub();
   ...
   */
    ...
    if (this.#cacheConfig === false) {
      ...
      const resolvedCache = this.#cacheConfig ?? this.#mastra?.serverCache ?? new chunkGLBBBSS5_cjs.InMemoryServerCache();
      this.#resolvedCache = resolvedCache;
      this.#cachingPubsub = new chunkW7D53JMX_cjs.CachingPubSub(this.#innerPubsub, resolvedCache);
    }
  }
  // ===========================================================================
