...
var CoreToolBuilder = class extends chunkFCQNDFEW_cjs.MastraBase {
  originalTool;
  options;
  logType;
  constructor(input) {
    ...
    if (!chunkM4KUR53T_cjs.isVercelTool(this.originalTool) && !chunkM4KUR53T_cjs.isProviderDefinedTool(this.originalTool)) {
      if (isBackgroundEligible || isResumableTool) {
        ...
        if (chunkRLPRSIR2_cjs.isZodObject(schema)) {
          let nextSchema = schema;
          if (isBackgroundEligible) {
            nextSchema = nextSchema.extend({
              _background: chunkEBDGUBJJ_cjs.backgroundOverrideZodSchema
            });
          }
          if (isResumableTool) {
            nextSchema = nextSchema.extend({
          ...
          const jsonSchema2 = chunkXB4FLS7A_cjs.standardSchemaToJSONSchema(schema, { io: "input" });
          if (jsonSchema2 && typeof jsonSchema2 === "object" && jsonSchema2.type === "object") {
            if (isBackgroundEligible) {
              jsonSchema2.properties = {
                ...jsonSchema2.properties,
                _background: chunkEBDGUBJJ_cjs.backgroundOverrideJsonSchema
              };
            }
            if (isResumableTool) {
              jsonSchema2.properties = {
            ...
            this.originalTool.inputSchema = chunkXB4FLS7A_cjs.toStandardSchema(jsonSchema2);
          }
        }
      }
    }
  ...
  getParameters = () => {
    if (chunkM4KUR53T_cjs.isVercelTool(this.originalTool)) {
  ...
  createExecute(tool, options, logType) {
    ...
    const mcpMeta = !chunkM4KUR53T_cjs.isVercelTool(tool) && "mcpMetadata" in tool ? tool.mcpMetadata : void 0;
    const execFunction = async (args, execOptions, toolSpan) => {
      try {
        let result;
        let suspendData = null;
        if (chunkM4KUR53T_cjs.isVercelTool(tool)) {
          result = await chunk5IS5R433_cjs.executeWithContext({
            span: toolSpan,
            fn: async () => tool?.execute?.(args, execOptions)
          });
        } else {
          const wrappedMastra = options.mastra ? chunkCDN65CUN_cjs.wrapMastra(options.mastra, { currentSpan: toolSpan }) : options.mastra;
          ...
          const baseContext = {
            ...
            runId: options.runId,
            requestContext: execOptions.requestContext ?? options.requestContext ?? new chunkKF4FCWWL_cjs.RequestContext(),
            ...
            ...chunkCDN65CUN_cjs.createObservabilityContext({ currentSpan: toolSpan }),
          ...
          if (resumeData) {
            const resumeValidation = chunkM4KUR53T_cjs.validateToolInput(resumeSchema, resumeData, options.name);
          ...
          result = await chunk5IS5R433_cjs.executeWithContext({ span: toolSpan, fn: async () => tool?.execute?.(args, toolContext) });
        }
        if (suspendData) {
          const suspendSchema = this.getSuspendSchema();
          const suspendValidation = chunkM4KUR53T_cjs.validateToolSuspendData(suspendSchema, suspendData, options.name);
        ...
        if (chunkM4KUR53T_cjs.isVercelTool(tool)) {
          ...
          const outputValidation = chunkM4KUR53T_cjs.validateToolOutput(outputSchema, result, options.name, false);
    ...
    return async (args, execOptions) => {
      ...
      const toolSpan = chunk5IS5R433_cjs.getOrCreateSpan({
        ...
        input: args,
        entityType: chunkEGOKFX44_cjs.EntityType.TOOL,
      ...
      try {
        ...
        const { data, error } = chunkM4KUR53T_cjs.validateToolInput(parameters, args, options.name);
  ...
  build() {
    ...
    if (chunkM4KUR53T_cjs.isVercelTool(this.originalTool) && "needsApproval" in this.originalTool) {
...
function ensureToolProperties(tools) {
  const toolsWithProperties = Object.keys(tools).reduce((acc, key) => {
    ...
    if (tool) {
      if (typeof tool === "function" && !(tool instanceof chunkM4KUR53T_cjs.Tool) && !chunkM4KUR53T_cjs.isVercelTool(tool)) {
...
//# sourceMappingURL=chunk-AX6D722J.cjs.map
//# sourceMappingURL=chunk-AX6D722J.cjs.map