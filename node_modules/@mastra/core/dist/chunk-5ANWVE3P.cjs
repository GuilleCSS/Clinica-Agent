...
var LocalProcessManager = class extends SandboxProcessManager {
  async spawn(command, options = {}) {
    ...
    if (options.cwd) {
      if (nodePath__namespace.isAbsolute(options.cwd)) {
        cwd = options.cwd;
      } else {
        const normalizedWorkingDir = nodePath__namespace.resolve(this.sandbox.workingDirectory);
        const normalizedOptionsCwd = nodePath__namespace.resolve(options.cwd);
        const isAlreadyWorkspacePath = normalizedOptionsCwd === normalizedWorkingDir || normalizedOptionsCwd.startsWith(`${normalizedWorkingDir}${nodePath__namespace.sep}`);
        cwd = isAlreadyWorkspacePath ? normalizedOptionsCwd : nodePath__namespace.resolve(this.sandbox.workingDirectory, options.cwd);
      }
    }
...
var MastraSandbox = class extends chunkFCQNDFEW_cjs.MastraBase {
  /** Process manager */
  processes;
...
var LocalSandbox = class extends MastraSandbox {
  ...
  addMountPathToIsolation(mountPath, hostPath) {
    ...
    try {
      isolationPath = fs.realpathSync(hostPath);
    } catch {
    }
...
var LocalSkillSource = class {
  ...
  async readdir(skillPath) {
    ...
    return Promise.all(
      entries.map(async (entry) => {
        const entryPath = nodePath__namespace.join(resolved, entry.name);
        const isSymlink = entry.isSymbolicLink();
  ...
  async realpath(skillPath) {
    return fs2__namespace.realpath(this.#resolvePath(skillPath));
  }
};
...
var VersionedSkillSource = class {
  ...
  async realpath(path9) {
    return this.#normalizePath(path9);
  }
};
...
var CompositeVersionedSkillSource = class {
  ...
  #normalizePath(path9) {
    let start = 0;
    while (start < path9.length) {
      const c = path9.charCodeAt(start);
      if (c === 46 || c === 47 || c === 92) {
        start++;
      } else {
        break;
    ...
    let end = path9.length;
    while (end > start) {
      const c = path9.charCodeAt(end - 1);
      if (c === 47 || c === 92) {
        end--;
      } else {
        break;
    ...
    return start === 0 && end === path9.length ? path9 : path9.slice(start, end);
  }
  /**
  ...
  async #isSkillsPathStale() {
    ...
    for (const skillsPath of this.#resolvedPaths) {
      ...
      for (const pathToCheck of pathsToCheck) {
        try {
          const stat4 = await this.#source.stat(pathToCheck);
          const mtime = stat4.modifiedAt.getTime();
...
function createSkillTool(skills) {
  const tool = chunkM4KUR53T_cjs.createTool({
    id: "skill",
...
function createSkillSearchTool(skills) {
  const tool = chunkM4KUR53T_cjs.createTool({
    ...
    inputSchema: v4.z.object({
      query: v4.z.string().describe("The search query"),
      skillNames: v4.z.array(v4.z.string()).optional().describe("Optional list of skill names to search within"),
      topK: v4.z.number().optional().describe("Maximum number of results to return (default: 5)")
    }),
...
var Workspace = class {
  ...
  constructor(config) {
    ...
    if (config.mounts && Object.keys(config.mounts).length > 0) {
      ...
      if (/^class\s/.test(Function.prototype.toString.call(config.filesystem))) {
        throw new WorkspaceError(
          "filesystem received a class constructor instead of an instance or resolver function. Pass an instance (e.g., new LocalFilesystem(...)) or a resolver function (({ requestContext }) => fs).",
          "INVALID_CONFIG"
        );
      }
    ...
    if (config.browser) {
      if (config.browser.providerType !== "cli") {
        throw new WorkspaceError(
          `Workspace.browser requires a CLI provider (providerType: 'cli'), but got '${config.browser.providerType}'. SDK providers should be used with Agent.browser instead.`,
          "INVALID_CONFIG",
          this.id
        );
      }
  ...
  async getInfo(options) {
    ...
    const filesystem = this._fs ?? (this._filesystemResolver ? await this.resolveFilesystem({ requestContext: options?.requestContext ?? new chunkKF4FCWWL_cjs.RequestContext() }) : void 0);
    if (filesystem) {
...
var astEditTool = chunkM4KUR53T_cjs.createTool({
...
Pattern replace (for everything else):
  ...
  inputSchema: v4.z.object({
    path: v4.z.string().describe("The path to the file to edit"),
    pattern: v4.z.string().optional().describe('AST pattern to search for (supports $VARIABLE placeholders, e.g., "console.log($ARG)")'),
    replacement: v4.z.string().optional().describe('Replacement pattern (can use captured $VARIABLES, e.g., "logger.debug($ARG)")'),
    transform: v4.z.enum(["add-import", "remove-import", "rename"]).optional().describe("Structured transformation to apply"),
    targetName: v4.z.string().optional().describe("Required for remove-import and rename transforms. The current name to target."),
    newName: v4.z.string().optional().describe("Required for rename transform. The new name to replace targetName with."),
    importSpec: v4.z.object({
      module: v4.z.string().describe("Module to import from"),
      names: v4.z.array(v4.z.string()).min(1).describe("Names to import. For default imports, put the default name first."),
...
var deleteFileTool = chunkM4KUR53T_cjs.createTool({
...
var editFileTool = chunkM4KUR53T_cjs.createTool({
...
- Use replace_all only when intentionally replacing all occurrences.`,
  inputSchema: v4.z.object({
    path: v4.z.string().describe("The path to the file to edit"),
    old_string: v4.z.string().describe("The exact text to find and replace. Must be unique in the file."),
...
// src/browser/cli-handler.ts
var CLI_CDP_PATTERNS = {
  "agent-browser": {
...
var browserCliHandler = new BrowserCliHandler();
...
var NUMERIC_TIMEOUT_STRING_REGEX = /^\d+(?:\.\d+)?$/;
var executeCommandInputSchema = v4.z.object({
  command: v4.z.string().describe('The shell command to execute (e.g., "npm install", "ls -la src/", "cat file.txt | grep error")'),
  timeout: v4.z.preprocess((value) => {
    if (typeof value !== "string") {
      return value;
    }
    const trimmed = value.trim();
    return NUMERIC_TIMEOUT_STRING_REGEX.test(trimmed) ? Number(trimmed) : value;
  }, v4.z.number()).nullish().describe("Maximum execution time in seconds. Example: 60 for 1 minute."),
  cwd: v4.z.string().nullish().describe("Working directory for the command"),
  tail: v4.z.number().nullish().describe(
...
var executeCommandWithBackgroundSchema = executeCommandInputSchema.extend({
  background: v4.z.boolean().optional().describe(
      ...
      for (const { cliName, command: warmupCmd } of warmups) {
        try {
          if (sandbox.executeCommand) {
            await sandbox.executeCommand(warmupCmd, [], { timeout: 1e4 });
          }
...
var executeCommandTool = chunkM4KUR53T_cjs.createTool({
...
var executeCommandWithBackgroundTool = chunkM4KUR53T_cjs.createTool({
  id: WORKSPACE_TOOLS.SANDBOX.EXECUTE_COMMAND,
  description: `${baseDescription}
...
var fileStatTool = chunkM4KUR53T_cjs.createTool({
...
var getProcessOutputTool = chunkM4KUR53T_cjs.createTool({
...
Use this after starting a background command with execute_command (background: true) to check if the process is still running and read its output.`,
  toModelOutput: sandboxToModelOutput,
  inputSchema: v4.z.object({
    pid: v4.z.string().describe("The process ID returned when the background command was started"),
    tail: v4.z.number().optional().describe(
    ...
    wait: v4.z.boolean().optional().describe(
...
var grepTool = chunkM4KUR53T_cjs.createTool({
...
- With context: { pattern: "function", contextLines: 2 }`,
  inputSchema: v4.z.object({
    pattern: v4.z.string().describe("Regex pattern to search for"),
    path: v4.z.string().optional().default(".").describe(
    ...
    contextLines: v4.z.number().optional().default(0).describe("Number of lines of context to include before and after each match (default: 0)"),
    maxCount: v4.z.number().optional().describe(
      "Maximum matches per file. Moves on to the next file after this many matches. Similar to grep -m flag."
    ),
    caseSensitive: v4.z.boolean().optional().default(true).describe("Whether the search is case-sensitive (default: true)"),
    includeHidden: v4.z.boolean().optional().default(false).describe('Include hidden files and directories (names starting with ".") in the search (default: false)')
  }),
...
var killProcessTool = chunkM4KUR53T_cjs.createTool({
...
Use this to stop a long-running background process that was started with execute_command (background: true). Returns the last ${KILL_TAIL_LINES} lines of output.`,
  toModelOutput: sandboxToModelOutput,
  inputSchema: v4.z.object({
    pid: v4.z.string().describe("The process ID of the background process to kill")
  }),
...
var listFilesTool = chunkM4KUR53T_cjs.createTool({
...
To list ALL files, omit the pattern parameter \u2014 do NOT pass pattern: "*".`,
  inputSchema: v4.z.object({
    path: v4.z.string().default(".").describe("Directory path to list"),
    maxDepth: v4.z.number().optional().default(2).describe("Maximum depth to descend (default: 2). Similar to tree -L flag."),
    showHidden: v4.z.boolean().optional().default(false).describe('Show hidden files starting with "." (default: false). Similar to tree -a flag.'),
    dirsOnly: v4.z.boolean().optional().default(false).describe("List directories only, no files (default: false). Similar to tree -d flag."),
    exclude: v4.z.string().optional().describe('Pattern to exclude (e.g., "node_modules"). Similar to tree -I flag.'),
    extension: v4.z.string().optional().describe('Filter by file extension (e.g., ".ts"). Similar to tree -P flag.'),
    pattern: v4.z.union([v4.z.string(), v4.z.array(v4.z.string())]).optional().describe(
...
var lspInspectTool = chunkM4KUR53T_cjs.createTool({
...
var mkdirTool = chunkM4KUR53T_cjs.createTool({
...
var readFileTool = chunkM4KUR53T_cjs.createTool({
  ...
  inputSchema: v4.z.object({
    path: v4.z.string().describe('The path to the file to read (e.g., "data/config.json")'),
    encoding: v4.z.enum(["utf-8", "utf8", "base64", "hex", "binary"]).optional().describe("The encoding to use when reading the file. Defaults to utf-8 for text files."),
    offset: v4.z.number().optional().describe("Line number to start reading from (1-indexed). If omitted, starts from line 1."),
...
var searchInputSchema = v4.z.object({
  query: v4.z.string().describe("The search query string"),
  topK: v4.z.number().optional().default(5).describe("Maximum number of results to return"),
  mode: v4.z.enum(["bm25", "vector", "hybrid"]).optional().describe("Search mode: bm25 for keyword search, vector for semantic search, hybrid for both combined"),
  minScore: v4.z.number().optional().describe("Minimum score threshold (0-1 for normalized scores)")
});
var searchTool = chunkM4KUR53T_cjs.createTool({
...
var writeFileTool = chunkM4KUR53T_cjs.createTool({
  ...
  inputSchema: v4.z.object({
    path: v4.z.string().describe('The path where to write the file (e.g., "data/output.txt")'),
...
function hasFilesystemConfig(workspace) {
  if (typeof workspace?.hasFilesystemConfig === "function") {
    return workspace.hasFilesystemConfig();
  }
...
async function resolveEffectiveWorkspace(workspace, context) {
  if (!workspace.filesystem && hasFilesystemConfig(workspace)) {
    const requestContext = context?.requestContext ?? new chunkKF4FCWWL_cjs.RequestContext();
...
function wrapTool(tool, workspace) {
  return {
    ...tool,
...
function wrapWithReadTracker(tool, workspace, readTracker, config, mode) {
  return {
    ...
    execute: async (input, context = {}) => {
      const effectiveWorkspace = await resolveEffectiveWorkspace(context?.workspace ?? workspace, context);
      let enrichedContext = { ...context, workspace: effectiveWorkspace };
      const fs6 = effectiveWorkspace.filesystem;
      if (mode === "write" && fs6) {
        ...
        try {
          const stat4 = await fs6.stat(input.path);
  ...
  if (workspace.canBM25 || workspace.canVector) {
    ...
    const dynamicSearchTool = {
      ...
      inputSchema: searchInputSchema.extend({
        mode: v4.z.enum(availableModes).optional().describe(`Search mode: ${availableModes.join(", ")}`)
      })
    };
...
exports.BrowserCliHandler = BrowserCliHandler;
...
exports.getTiktoken = getTiktoken;
