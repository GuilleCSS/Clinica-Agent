...
var chunkD6H6ESH4_cjs = require('./chunk-D6H6ESH4.cjs');
...
var AIV5Adapter = class {
  ...
  static fromUIMessage(uiMsg) {
    ...
    const toolInvocationParts = parts.filter((p) => chunk7KR4BWEU_cjs.isToolUIPart(p));
    ...
    const v2Parts = parts.map((p) => {
      if (chunk7KR4BWEU_cjs.isToolUIPart(p)) {
        const toolName = getToolName(p);
...
var AIV6Adapter = class _AIV6Adapter {
  static toUIMessage(dbMsg) {
    ...
    if (!hasToolInvocationParts || !hasReasoningParts || !hasFileParts || !hasTextParts) {
      for (const part of v5Message.parts) {
        if (chunk7KR4BWEU_cjs.isToolUIPart(part)) {
  ...
  static fromUIMessage(uiMsg) {
    const compatibleParts = uiMsg.parts.filter((part) => {
      ...
      if (chunkMWU53QPO_cjs.isToolUIPart(part) && isV6OnlyToolState(part.state)) return false;
      return true;
    });
    ...
    for (const part of uiMsg.parts) {
      ...
      if (!chunkMWU53QPO_cjs.isToolUIPart(part) || !isV6OnlyToolState(part.state)) {
...
var CacheKeyGenerator = class _CacheKeyGenerator {
  ...
  static fromAIV5Parts(parts) {
    ...
    for (const part of parts) {
      ...
      if (chunk7KR4BWEU_cjs.isToolUIPart(part) || part.type === "dynamic-tool") {
        key += part.toolCallId;
        key += part.state;
      }
      if (part.type === "reasoning") {
...
function sanitizeV5UIMessages(messages, filterIncompleteToolCalls = false) {
  const msgs = messages.map((m) => {
    ...
    const safeParts = m.parts.filter((p) => {
      ...
      if (!chunk7KR4BWEU_cjs.isToolUIPart(p)) return true;
    ...
    const sanitized = {
      ...
      parts: mergedParts.map((part) => {
        if (chunk7KR4BWEU_cjs.isToolUIPart(part) && part.state === "output-available") {
          return {
            ...part,
...
function addStartStepPartsForAIV5(messages) {
  for (const message of messages) {
    ...
    for (const [index, part] of message.parts.entries()) {
      if (!chunk7KR4BWEU_cjs.isToolUIPart(part)) continue;
      const nextPart = message.parts.at(index + 1);
      if (nextPart && nextPart.type !== `step-start` && !chunk7KR4BWEU_cjs.isToolUIPart(nextPart)) {
        message.parts.splice(index + 1, 0, { type: "step-start" });
      }
      if (nextPart && chunk7KR4BWEU_cjs.isToolUIPart(nextPart) && !part.providerExecuted && nextPart.providerExecuted && (nextPart.state === "output-available" || nextPart.state === "output-error")) {
        message.parts.splice(index + 1, 0, { type: "step-start" });
      }
    }
  }
...
function aiV4UIMessagesToAIV4CoreMessages(messages) {
  return chunkD6H6ESH4_cjs.convertToCoreMessages(sanitizeAIV4UIMessages(messages));
}
...
function aiV5UIMessagesToAIV5ModelMessages(messages, dbMessages, filterIncompleteToolCalls = false) {
  ...
  const result = restoreAssistantFileProviderMetadata(chunk7KR4BWEU_cjs.convertToModelMessages(preprocessed), preprocessed);
...
var StepContentExtractor = class _StepContentExtractor {
  ...
  static convertPartsToContent(parts, stepId, stepContentFn) {
    ...
    const modelMessages = chunk7KR4BWEU_cjs.convertToModelMessages(sanitizeV5UIMessages(stepUiMessages));
...
var MessageList = class {
      ...
      for (let i = 0; i < msg.content.parts.length; i++) {
        ...
        if (part?.type === "tool-invocation" && part.toolInvocation?.toolCallId === toolCallId) {
          ...
          msg.content.metadata = {
            ...existingMeta,
            ...incomingMeta,
            ...existingBgTasks || incomingBgTasks ? { backgroundTasks: { ...existingBgTasks ?? {}, ...incomingBgTasks ?? {} } } : {}
          };
  ...
  addOne(message, messageSource) {
    ...
    const shouldMerge = latestMessageIsAfterSealedBoundary && !hasSealedReplacementTarget && MessageMerger.shouldMerge(latestMessage, messageV2, messageSource, isLatestFromMemory, this._agentNetworkAppend);
...
//# sourceMappingURL=chunk-B5SLBAGA.cjs.map
//# sourceMappingURL=chunk-B5SLBAGA.cjs.map