...
function toScorerTargetEntityType(entityType) {
  switch (entityType) {
    case "AGENT":
      return chunkEGOKFX44_cjs.EntityType.AGENT;
    case "WORKFLOW":
      return chunkEGOKFX44_cjs.EntityType.WORKFLOW_RUN;
    default:
      return void 0;
  }
}
...
async function validateAndSaveScore(storage, payload) {
  ...
  const payloadToSave = chunkXXKOQCB4_cjs.saveScorePayloadSchema.parse(payload);
...
function isWorkflow(target) {
  return target instanceof chunkQPZ35KK2_cjs.Workflow;
}
...
async function executeWorkflow(target, item, targetOptions) {
  const observabilityContext = chunkCDN65CUN_cjs.resolveObservabilityContext(item);
  ...
  return {
    ...
    entityType: chunkEGOKFX44_cjs.EntityType.WORKFLOW_RUN,
    scoringData: {
      input: item.input,
...
async function executeAgent(agent, item, targetOptions) {
  ...
  if (chunkQPZ35KK2_cjs.isSupportedLanguageModel(model)) {
    ...
    return {
      ...result,
      entityType: chunkEGOKFX44_cjs.EntityType.AGENT
    };
  } else {
...
async function extractTrajectoryFromTraceStore(storage, traceId, spanId) {
  ...
  try {
    ...
    return chunkXXKOQCB4_cjs.extractTrajectoryFromTrace(trace.spans, spanId);
  } catch {
    return void 0;
  }
}
async function runScorers(scorers, targetResult, item, storage) {
  ...
  if (Array.isArray(scorers)) {
    ...
    if (scorers.trajectory) {
      ...
      const trajectory = traceTrajectory ?? (rawOutput ? chunkXXKOQCB4_cjs.extractTrajectory(rawOutput) : { steps: [] });
          ...
          for (const scorer of stepScorers) {
            try {
              const score = await scorer.run({
                ...
                scoreSource: "experiment",
                targetScope: "span",
                targetEntityType: chunkEGOKFX44_cjs.EntityType.WORKFLOW_STEP,
                targetTraceId
              });
    ...
    if (scorers.trajectory) {
      ...
      if (!trajectory) {
        ...
        trajectory = stepResults ? chunkXXKOQCB4_cjs.extractWorkflowTrajectory(stepResults, stepExecutionPath) : { steps: [] };
      }
      for (const scorer of scorers.trajectory) {
        try {
          const score = await scorer.run({
            ...
            scoreSource: "experiment",
            targetScope: "trajectory",
            targetEntityType: chunkEGOKFX44_cjs.EntityType.TRAJECTORY,
            targetTraceId,
            targetSpanId: targetResult.spanId
          });
          trajectoryScorerResults[scorer.id] = score;
...
//# sourceMappingURL=chunk-MT42UWQD.cjs.map
//# sourceMappingURL=chunk-MT42UWQD.cjs.map