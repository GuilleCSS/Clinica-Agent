...
var chunkDDVJ7BU5_cjs = require('./chunk-DDVJ7BU5.cjs');
var chunk5SWW6MXD_cjs = require('./chunk-5SWW6MXD.cjs');
var chunkDFDUKIQU_cjs = require('./chunk-DFDUKIQU.cjs');
var chunkLIDHQ5MH_cjs = require('./chunk-LIDHQ5MH.cjs');
var chunk5JXIKYBM_cjs = require('./chunk-5JXIKYBM.cjs');
var chunk52AOKVLY_cjs = require('./chunk-52AOKVLY.cjs');
var chunk4ZG6GODQ_cjs = require('./chunk-4ZG6GODQ.cjs');
var chunkA3NDWEDZ_cjs = require('./chunk-A3NDWEDZ.cjs');
...
var ObservabilityStorage = class _ObservabilityStorage extends chunkA3NDWEDZ_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "OBSERVABILITY"
  ...
  async getBranch(args) {
    const parsed = chunkXXKOQCB4_cjs.getBranchArgsSchema.parse(args);
    try {
      const skeleton = await this.getStructure({ traceId: parsed.traceId });
      if (!skeleton) return null;
      const branchSpanIds = chunkXXKOQCB4_cjs.extractBranchSpans(skeleton.spans, parsed.spanId, parsed.depth).map((s) => s.spanId);
      ...
      const isFallbackTrigger = error instanceof chunk4U7ZLI36_cjs.MastraError && (error.id === "OBSERVABILITY_STORAGE_GET_STRUCTURE_NOT_IMPLEMENTED" || error.id === "OBSERVABILITY_STORAGE_GET_TRACE_LIGHT_NOT_IMPLEMENTED" || error.id === "OBSERVABILITY_STORAGE_GET_SPANS_NOT_IMPLEMENTED");
      if (!isFallbackTrigger) throw error;
    }
    const trace = await this.getTrace({ traceId: parsed.traceId });
    if (!trace) return null;
    const spans = chunkXXKOQCB4_cjs.extractBranchSpans(trace.spans, parsed.spanId, parsed.depth);
...
function transformRow(row, tableName, options = {}) {
  ...
  const tableSchema = chunkXXKOQCB4_cjs.TABLE_SCHEMAS[tableName];
  const result = {};
...
function transformScoreRow(row, options = {}) {
  return transformRow(row, chunkXXKOQCB4_cjs.TABLE_SCORERS, options);
}
...
var ObservabilityInMemory = class extends ObservabilityStorage {
  ...
  async listTraces(args) {
    const { filters, pagination, orderBy } = chunkXXKOQCB4_cjs.listTracesArgsSchema.parse(args);
    ...
    return {
      spans: chunkXXKOQCB4_cjs.toTraceSpans(paged),
  ...
  async listBranches(args) {
    const { filters, pagination, orderBy } = chunkXXKOQCB4_cjs.listBranchesArgsSchema.parse(args);
    const allowedSpanTypes = filters?.spanType ? chunkXXKOQCB4_cjs.BRANCH_SPAN_TYPE_SET.has(filters.spanType) ? /* @__PURE__ */ new Set([filters.spanType]) : /* @__PURE__ */ new Set() : chunkXXKOQCB4_cjs.BRANCH_SPAN_TYPE_SET;
    const matches = [];
    for (const [, traceEntry] of this.db.traces) {
    ...
    return {
      pagination: { total, page, perPage, hasMore: end < total },
      branches: paged.map(chunkXXKOQCB4_cjs.toTraceSpan)
    };
  }
  /**
  ...
  spanMatchesBranchFilters(span, filters) {
    ...
    if (filters.status !== void 0) {
      const spanStatus = chunkXXKOQCB4_cjs.toTraceSpan(span).status;
  ...
  async listMetrics(args) {
    const { filters, pagination, orderBy } = chunkEGOKFX44_cjs.listMetricsArgsSchema.parse(args);
  ...
  async getEntityTypes(_args) {
    const validTypes = new Set(Object.values(chunkEGOKFX44_cjs.EntityType));
  ...
  async listLogs(args) {
    const { filters, pagination, orderBy } = chunkEGOKFX44_cjs.listLogsArgsSchema.parse(args);
  ...
  async listScores(args) {
    const { filters, pagination, orderBy } = chunkEGOKFX44_cjs.listScoresArgsSchema.parse(args);
  ...
  async listFeedback(args) {
    const { filters, pagination, orderBy } = chunkEGOKFX44_cjs.listFeedbackArgsSchema.parse(args);
...
var entityTypeValues = new Set(Object.values(chunkEGOKFX44_cjs.EntityType));
...
var BackgroundTasksStorage = class extends chunkA3NDWEDZ_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
...
var BlobStore = class extends chunkFCQNDFEW_cjs.MastraBase {
  constructor() {
    super({
      component: "STORAGE",
      name: "BLOBS"
...
var ChannelsStorage = class extends chunkA3NDWEDZ_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "CHANNELS"
...
var DatasetsStorage = class extends chunkA3NDWEDZ_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "DATASETS"
...
var DatasetsInMemory = class extends DatasetsStorage {
  ...
  async listDatasets(args) {
    ...
    const perPage = chunkA3NDWEDZ_cjs.normalizePerPage(perPageInput, 100);
    const { offset: start, perPage: perPageForResponse } = chunkA3NDWEDZ_cjs.calculatePagination(page, perPageInput, perPage);
...
var ExperimentsStorage = class extends chunkA3NDWEDZ_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "EXPERIMENTS"
...
var MemoryStorage = class extends chunkA3NDWEDZ_cjs.StorageDomain {
  /**
...
var InMemoryMemory = class extends MemoryStorage {
  ...
  async listMessages({
    ...
    const perPage = chunkA3NDWEDZ_cjs.normalizePerPage(perPageInput, 40);
    if (page < 0) {
      throw new Error("page must be >= 0");
    }
    ...
    const { offset, perPage: perPageForResponse } = chunkA3NDWEDZ_cjs.calculatePagination(page, perPageInput, perPage);
  ...
  async listMessagesByResourceId({
    ...
    const list = new chunkB5SLBAGA_cjs.MessageList().add(
  ...
  async saveMessages(args) {
    ...
    const list = new chunkB5SLBAGA_cjs.MessageList().add(messages, "memory");
    return { messages: list.get.all.db() };
  }
...
var SchedulesStorage = class extends chunkA3NDWEDZ_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "SCHEDULES"
...
var ScoresStorage = class extends chunkA3NDWEDZ_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "SCORES"
...
var ScoresInMemory = class extends ScoresStorage {
  ...
  async listScoresByScorerId({
    ...
    const perPage = chunkA3NDWEDZ_cjs.normalizePerPage(perPageInput, Number.MAX_SAFE_INTEGER);
    const { offset: start, perPage: perPageForResponse } = chunkA3NDWEDZ_cjs.calculatePagination(page, perPageInput, perPage);
...
var WorkflowsStorage = class extends chunkA3NDWEDZ_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "WORKFLOWS"
...
var WorkflowsInMemory = class extends WorkflowsStorage {
  ...
  async listWorkflowRuns({
    ...
    if (perPage !== void 0 && page !== void 0) {
      const normalizedPerPage = chunkA3NDWEDZ_cjs.normalizePerPage(perPage, Number.MAX_SAFE_INTEGER);
...
// src/storage/mock.ts
var InMemoryStore = class extends chunkA3NDWEDZ_cjs.MastraCompositeStore {
  stores;
  /**
  ...
  constructor({ id = "in-memory" } = {}) {
    ...
    this.stores = {
      ...
      agents: new chunk5SWW6MXD_cjs.InMemoryAgentsStorage({ db: this.#db }),
      ...
      promptBlocks: new chunk5JXIKYBM_cjs.InMemoryPromptBlocksStorage({ db: this.#db }),
      scorerDefinitions: new chunk52AOKVLY_cjs.InMemoryScorerDefinitionsStorage({ db: this.#db }),
      mcpClients: new chunkDFDUKIQU_cjs.InMemoryMCPClientsStorage({ db: this.#db }),
      mcpServers: new chunkLIDHQ5MH_cjs.InMemoryMCPServersStorage({ db: this.#db }),
      workspaces: new chunkDDVJ7BU5_cjs.InMemoryWorkspacesStorage({ db: this.#db }),
      skills: new chunk4ZG6GODQ_cjs.InMemorySkillsStorage({ db: this.#db }),
...
var FilesystemStore = class extends chunkA3NDWEDZ_cjs.MastraCompositeStore {
  #db;
  #dir;
  constructor(config = {}) {
    ...
    this.stores = {
      agents: new chunk5SWW6MXD_cjs.FilesystemAgentsStorage({ db: this.#db }),
      promptBlocks: new chunk5JXIKYBM_cjs.FilesystemPromptBlocksStorage({ db: this.#db }),
      scorerDefinitions: new chunk52AOKVLY_cjs.FilesystemScorerDefinitionsStorage({ db: this.#db }),
      mcpClients: new chunkDFDUKIQU_cjs.FilesystemMCPClientsStorage({ db: this.#db }),
      mcpServers: new chunkLIDHQ5MH_cjs.FilesystemMCPServersStorage({ db: this.#db }),
      workspaces: new chunkDDVJ7BU5_cjs.FilesystemWorkspacesStorage({ db: this.#db }),
      skills: new chunk4ZG6GODQ_cjs.FilesystemSkillsStorage({ db: this.#db })
    };
  }
  /**
...
var StoreOperations = class extends chunkFCQNDFEW_cjs.MastraBase {
  constructor() {
    super({
      component: "STORAGE",
      name: "OPERATIONS"
...
var StoreOperationsInMemory = class extends StoreOperations {
  ...
  async insert({ tableName, record }) {
    ...
    let key = record.id;
    if ([chunkXXKOQCB4_cjs.TABLE_WORKFLOW_SNAPSHOT].includes(tableName) && !record.id && record.run_id) {
...
//# sourceMappingURL=chunk-B7CR6B45.cjs.map
//# sourceMappingURL=chunk-B7CR6B45.cjs.map