...
async function executeAgent(agent, item, signal, requestContext, experimentId, versions) {
  ...
  const rawResult = chunkQPZ35KK2_cjs.isSupportedLanguageModel(model) ? await agent.generate(input, {
    scorers: {},
...
async function executeWorkflow(workflow, item, requestContext) {
  const reqCtx = requestContext ? new chunkKF4FCWWL_cjs.RequestContext(Object.entries(requestContext)) : void 0;
  const observabilityContext = chunkCDN65CUN_cjs.resolveObservabilityContext({});
...
function toScorerTargetEntityType(targetType) {
  switch (targetType) {
    ...
    case "scorer":
      return chunkEGOKFX44_cjs.EntityType.SCORER;
    default:
      return void 0;
  }
}
...
async function extractTrajectoryFromStorage(storage, traceId) {
  ...
  try {
    ...
    return chunkXXKOQCB4_cjs.extractTrajectoryFromTrace(trace.spans);
  } catch {
    return void 0;
  }
}
async function runScorersForItem(scorers, item, output, storage, runId, targetType, targetId, itemId, scorerInput, scorerOutput, traceId, workflowData) {
  ...
  if (hasTrajectoryScorer) {
    ...
    trajectoryOutput = traceTrajectory ?? (scorerOutput ? chunkXXKOQCB4_cjs.extractTrajectory(scorerOutput) : { steps: [] });
  }
  ...
  const settled = await Promise.allSettled(
    scorers.map(async (scorer) => {
      ...
      if (storage && result.score !== null) {
        try {
          await chunkMT42UWQD_cjs.validateAndSaveScore(storage, {
            scorerId: scorer.id,
            score: result.score,
...
async function runStepScorersForItem(stepScorers, item, workflowData, storage, runId, targetType, targetId, itemId, traceId) {
  ...
  for (const stepId of stepIds) {
    ...
    const targetCorrelationContext = {
      ...traceId ? { traceId } : {},
      entityType: chunkEGOKFX44_cjs.EntityType.WORKFLOW_STEP,
      entityId: stepId,
      entityName: stepId,
      experimentId: runId
    };
...
//# sourceMappingURL=chunk-YQ4V5JQE.cjs.map
//# sourceMappingURL=chunk-YQ4V5JQE.cjs.map