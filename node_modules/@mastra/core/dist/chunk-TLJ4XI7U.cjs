...
var chunkIX34UNHY_cjs = require('./chunk-IX34UNHY.cjs');
var chunkECN5FG7P_cjs = require('./chunk-ECN5FG7P.cjs');
var chunkVNYBKO6S_cjs = require('./chunk-VNYBKO6S.cjs');
var chunkYQ4V5JQE_cjs = require('./chunk-YQ4V5JQE.cjs');
var chunkMT42UWQD_cjs = require('./chunk-MT42UWQD.cjs');
...
var Mastra = class {
  ...
  get datasets() {
    if (!this.#datasets) {
      this.#datasets = new chunkYQ4V5JQE_cjs.DatasetsManager(this);
    }
  ...
  registerExporter(exporter, instance, entrypoint) {
    if (this.#observability instanceof chunkCDN65CUN_cjs.NoOpObservability) {
   ...
   */
  constructor(config) {
    chunkVNYBKO6S_cjs.initContextStorage();
    this.#serverCache = config?.cache ?? new chunkGLBBBSS5_cjs.InMemoryServerCache();
    ...
    if (config?.pubsub) {
      ...
      this.#pubsub = new chunkHM6XXOV2_cjs.EventEmitterPubSub();
    }
    this.#events = {};
    ...
    const workflowEventProcessor = new chunkIX34UNHY_cjs.WorkflowEventProcessor({ mastra: this });
    ...
    if (config?.logger === false) {
      logger = chunkSQXMPXUI_cjs.noopLogger;
    } else {
      if (config?.logger) {
        logger = config.logger;
      } else {
        const levelOnEnv = process.env.NODE_ENV === "production" && process.env.MASTRA_DEV !== "true" ? chunk7GW2TQXP_cjs.LogLevel.WARN : chunk7GW2TQXP_cjs.LogLevel.INFO;
        logger = new chunk7GW2TQXP_cjs.ConsoleLogger({ name: "Mastra", level: levelOnEnv });
      }
    }
    this.#logger = logger;
    ...
    if (storage) {
      storage = chunkQPZ35KK2_cjs.augmentWithInit(storage);
    }
    if (config?.observability) {
      if (typeof config.observability.getDefaultInstance === "function") {
        ...
        this.#observability = new chunkCDN65CUN_cjs.NoOpObservability();
      }
    } else {
      this.#observability = new chunkCDN65CUN_cjs.NoOpObservability();
    }
    const dualLogger = new chunkSQXMPXUI_cjs.DualLogger(this.#logger, () => this.loggerVNext);
    ...
    for (const gateway of chunkLUS5VPK2_cjs.defaultGateways) {
    ...
    chunkMR7ZWBL6_cjs.registerHook("onScorerRun" /* ON_SCORER_RUN */, chunkMT42UWQD_cjs.createOnScorerHook(this));
    this.#observability.setMastraContext({ mastra: this });
    ...
    const bgManager = new chunkEBDGUBJJ_cjs.BackgroundTaskManager(this.#backgroundTaskConfig);
  ...
  #shouldEnableScheduler() {
    ...
    this.#schedulerInitPromise = this.#initScheduler().catch((error) => {
      ...
      this.#schedulerInitPromise = void 0;
  ...
  async #initScheduler() {
    ...
    const scheduler = new chunkQPZ35KK2_cjs.WorkflowScheduler({
      schedulesStore,
      pubsub: this.#pubsub,
      config: this.#schedulerConfig
    });
    if (this.#logger) {
      scheduler.__setLogger(this.#logger);
    }
      ...
      try {
        ...
        if (!existing) {
          await schedulesStore.createSchedule({
            ...
            status: "active",
            nextFireAt: chunkQPZ35KK2_cjs.computeNextFireAt(cfg.cron, { timezone: cfg.timezone, after: now }),
            createdAt: now,
            updatedAt: now,
        ...
        if (cronChanged || timezoneChanged) {
          patch.nextFireAt = chunkQPZ35KK2_cjs.computeNextFireAt(cfg.cron, { timezone: cfg.timezone, after: now });
        }
  ...
  getChannels() {
    ...
    for (const [agentKey, agent] of Object.entries(this.#agents ?? {})) {
      ...
      if (agentChannels instanceof chunkQPZ35KK2_cjs.AgentChannels) {
  ...
  addAgent(agent, key, options) {
    ...
    if (chunkQPZ35KK2_cjs.isDurableAgentLike(agent)) {
    ...
    let mastraAgent;
    if (chunkECN5FG7P_cjs.isToolLoopAgentLike(agent)) {
      mastraAgent = chunkECN5FG7P_cjs.toolLoopAgentToMastraAgent(agent, { fallbackName: key });
    } else {
      mastraAgent = agent;
    }
   ...
   */
  setStorage(storage) {
    this.#storage = chunkQPZ35KK2_cjs.augmentWithInit(storage);
    this.#ensureBackgroundTaskManager();
    this.#ensureScheduler();
  }
  setLogger({ logger }) {
    const dualLogger = new chunkSQXMPXUI_cjs.DualLogger(logger, () => this.loggerVNext);
    this.#logger = dualLogger;
    if (this.#agents) {
   ...
   */
  get loggerVNext() {
    return this.#observability.getDefaultInstance()?.getLoggerContext?.() ?? chunkCDN65CUN_cjs.noOpLoggerContext;
  }
  /**
   ...
   */
  get metrics() {
    return this.#observability.getDefaultInstance()?.getMetricsContext?.() ?? chunkCDN65CUN_cjs.noOpMetricsContext;
  }
  getServerMiddleware() {
  ...
  #syncGatewayRegistry() {
    try {
      ...
      import('./provider-registry-UM6NWWSW.cjs').then(async ({ GatewayRegistry }) => {
...
//# sourceMappingURL=chunk-TLJ4XI7U.cjs.map
//# sourceMappingURL=chunk-TLJ4XI7U.cjs.map