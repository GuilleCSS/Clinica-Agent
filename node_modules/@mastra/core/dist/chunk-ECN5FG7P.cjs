...
function isToolLoopAgentLike(obj) {
  if (!obj) return false;
  if (obj instanceof chunkMWU53QPO_cjs.ToolLoopAgent) return true;
  ...
  async handlePrepareStep(args, currentResult) {
    if (this.settings.prepareStep) {
      ...
      if (currentResult.model) {
        const resolvedModel = await chunkLUS5VPK2_cjs.resolveModelConfig(currentResult.model);
        if (!chunkQPZ35KK2_cjs.isSupportedLanguageModel(resolvedModel)) {
...
function toolLoopAgentToMastraAgent(agent, options) {
  ...
  const id = agentConfig.id || options?.fallbackName || `tool-loop-agent-${chunkQPFIMRQM_cjs.generateId()}`;
  return new chunkQPZ35KK2_cjs.Agent({
    ...agentConfig,
    id,
...
//# sourceMappingURL=chunk-ECN5FG7P.cjs.map
//# sourceMappingURL=chunk-ECN5FG7P.cjs.map