...
var require_utils = chunkDZUJEN5N_cjs.__commonJS({
...
var require_schemes = chunkDZUJEN5N_cjs.__commonJS({
...
var require_fast_uri = chunkDZUJEN5N_cjs.__commonJS({
...
var JsonSchemaWrapper = class {
  ...
  #validate(value) {
    try {
      ...
      if (validateFn.$async) {
        return result.then(() => ({ value })).catch((err) => {
          const errors = err?.errors ?? [];
          const issues2 = errors.map((e) => ({
      ...
      const issues = (validateFn.errors ?? []).map((err) => ({
